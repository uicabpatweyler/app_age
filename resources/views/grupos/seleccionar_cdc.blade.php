...
@section('title', 'Seleccionar Cuota de Colegiatura')
            ...
            <div class="col-md-12">
                ...
                <div class="box box-success">
                    <div class="box-header with-border">
                        ...
                        <h3 class="box-title"> Seleccionar cuota de colegiatura</h3><small>&nbsp;&nbsp;(Los campos marcados con (*) son obligatorios)</small>
                    </div>
                    <!-- /.box-header -->
                    <!-- form start -->
                    <form class="form-horizontal" method="post" action="" name="form_asignar_cdc" id="form_asignar_cdc">
                        {{csrf_field()}}
                        ...
                        <div class="box-body">
                            ...
                            <div class="form-group">
                                <label for="cuotacolegiatura_id" class="col-sm-2 control-label"><p class="text-left">Cuota de Colegiatura:(*)</p></label>
                                <div class="col-sm-5">
                                    @if($grupo_cdc===null)
                                        <select id="cuotacolegiatura_id" name="cuotacolegiatura_id" class="form-control cuotacolegiatura_id" style="width: 100%;">
                                            <option value="-1" selected>[Seleccione una cuota de colegiatura]</option>
                                            @foreach($cuotas as $cuota)
                                                <option value="{{$cuota->id}}">{{$cuota->cuotacolegiatura_nombre}} ( $ {{number_format($cuota->cuotacolegiatura_cuota, 2, '.', ',')}} )</option>
                                            @endforeach
                                        </select>
                                    @else
                                        <select id="cuotacolegiatura_id" name="cuotacolegiatura_id" class="form-control cuotacolegiatura_id" style="width: 100%;">
                                            @foreach($cuotas as $cuota)
                                                @if($cuota->id===$grupo_cdc->cuotacolegiatura_id)
                                                    <option value="{{$cuota->id}}" selected>{{$cuota->cuotacolegiatura_nombre}} ( $ {{number_format($cuota->cuotacolegiatura_cuota, 2, '.', ',')}} )</option>
                                                @else
                                                    <option value="{{$cuota->id}}">{{$cuota->cuotacolegiatura_nombre}} ( $ {{number_format($cuota->cuotacolegiatura_cuota, 2, '.', ',')}} )</option>
                                                @endif
                                            @endforeach
...
@section('scripts')
    <script>
        $(document).ready(function () {
            $('.cuotacolegiatura_id').select2({
                allowClear: true,
                placeholder: {
                    id: "-1",
                    text: '[Seleccione una cuota de colegiatura]'
                }
            });

            $("#boton_enviar").click(function(){

                var cuotacolegiatura_id = $('#cuotacolegiatura_id').val();

                if(cuotacolegiatura_id==="-1"){
                    swal({
                        title:"Error:",
                        text: "Es necesario elegir una cuota de colegiatura",
                        type: "error",
            ...
            function ajaxSubmit(){
                $.ajax({
                    ...
                    data: $("#form_asignar_cdc").serialize(),
                    dataType : 'json',
